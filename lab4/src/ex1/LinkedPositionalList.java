...
public class LinkedPositionalList<E> implements PositionalList<E> {
  ...
  //task 1 of lab 4 below
  public int indexOf(Position<E> p)
  {
      int index = 0;// to keep track of the current index #
      Position<E> currentIndex = first();// to start at the first index position
      while (currentIndex != null)//continue the list until it hits the end
      {
          if (currentIndex == p)//if the position is the one that is needed
          {
              return index;// return the index
          }
          currentIndex = after(currentIndex);// move to the next one
          index++;// increases the counter by 1
      }
      return -1;
  }
    public static void main(String[] args)
    {
        LinkedPositionalList<String> list = new LinkedPositionalList<>();//create new positional list
        Position<String> p1 = list.addFirst("First");//the next 3 lines are the same as this one add the " " to the list and store its position
        Position<String> p2 = list.addLast("Second");
        Position<String> p3 = list.addLast("Third");
        System.out.println(list.indexOf(p1));//next 3 lines are the same it jsut prints the positions
