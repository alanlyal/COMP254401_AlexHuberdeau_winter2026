...
protected void upheap(int j){
    if(j == 0)
    {
        return;// if it hits root cancel the function
    }
    int p = parent(j); // give p the parent of j
    if (compare(heap.get(j), heap.get(p)) <0) //if j is smaller then p
    {
        swap(j,p);// swap there position
        upheap(p);// rerun the function
    }
}
