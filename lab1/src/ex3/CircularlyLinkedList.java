...
public class CircularlyLinkedList<E> {
  ...
  {
      CircularlyLinkedList<E> cloneList = new CircularlyLinkedList<>();//create a new but empty list
      if (tail == null)// just incase if the og list is empty, it will return the empty list
      {
          return cloneList;
      }
      Node<E> walk = tail.getNext();// to start from the first node
      cloneList.addLast(walk.getElement());// add the first cloned element to the clone list
      walk = walk.getNext();// move to the next node
      while(walk != tail.getNext())//this while will keep the copy loop until we hit the tail
      {
          cloneList.addLast(walk.getElement());
          walk=walk.getNext();
      }
      return cloneList;// this needs the function with the cloned list
  }
//main method
  public static void main(String[] args)
  {
      ...
      CircularlyLinkedList<String> copylist = circularList.clone();
