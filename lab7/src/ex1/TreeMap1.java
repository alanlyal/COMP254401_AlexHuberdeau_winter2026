...
public class TreeMap1<K,V> extends AbstractSortedMap<K,V> { 
	...
	{
		Position<Entry<K,V>> search = p; // start the search at root
        while(isInternal(search)) //loop to keep seaching while not in a leaf node
        {
            int compareNode = compare(key,search.getElement());// to look and compare the key with the current node
            if (compareNode == 0) //if the key is correct
            {
                return search;// end search
            } else if (compareNode < 0) // if the key is smaller(not correct key)
            {
                search = left(search);//move to left child
            }
            else// of the key is bigger (not the correct key)
            {
                search = right(search); //move to right child
            }
        }
        return search;// if leaf is hit cancel the function
	}
